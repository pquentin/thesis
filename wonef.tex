...
% TODO expliciter l'intérêt de ce travail

Le projet WordNet a commencé en 1985 en se basant sur la double observation
suivante à propos des dictionnaires de l'époque (tel que l'\emph{Oxford English
Dictionary}) \citep{miller1990introduction}:

\begin{itemize}

    \item ces dictionnaires, même dans leur version en ligne, ne tirent pas
profit des nouveaux moyens d'organisation rendus possibles avec un ordinateur ;

    \item ils sont basés sur l'histoire des mots au lieu de considérer les
progrès en linguistique et psycholinguistique proposant des manières plus
utiles d'organiser les mots.

\end{itemize}

Le résultat est la base de données lexicale WordNet \citep{fellbaum1998wordnet}
qui regroupe les mots en ensemble de synonymes (\emph{synonym sets} ou
\emph{synsets}) et qui lie les synsets entre eux par différentes relations
sémantiques telles que l'antonymie, l'hypernymie, l'hyponymie ou encore la
méronymie.

Depuis la fin des années 90, Il n'existe encore
que peu d'équivalents de même qualité dans d'autres langues
\citep{bond2012survey}.  Néanmoins, des traductions automatiques de WordNet
vers de nombreuses langues cibles existent, notamment pour le français.  % TODO
expliquer que WoNeF is the new JAWS JAWS est une telle traduction automatique
des noms de WordNet qui utilise des dictionnaires et un modèle de langue
syntaxique. Nous améliorons cette traduction, la complétons avec les verbes,
adjectifs et adverbes de WordNet, et démontrons la validité de notre approche
via une nouvelle évaluation manuelle.  En plus de la version principale nommée
WoNeF, nous produisons deux versions supplémentaires : une version à haute
précision (93~\% de précision, jusqu'à 97~\% pour les noms), et une version à
haute couverture contenant 109~447 paires (littéral, synset).
...
% TODO exemples fondamentaux ici
