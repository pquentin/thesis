...
V NP.Theme. Ici, dans cette constructions transitive (NP V NP, e.g. \emph{Sally
...
est \texttt{say-37.3-1}, mais il ne nous est pas possible de le déterminer
...
\begin{enumerate}
    ...
    \item Conserver les candidats qui sont les sujets les plus proches à gauche du prédicat et dont les relations du prédicat p à la tête t du candidat sont toutes montantes (g $\rightarrow$ p).
...
\citep{gildea2002automatic,das2014frame}. Ici, on commence par identifier les
frames possibles, ce qui restreint le nombre de classes VerbNet applicables. En
effet, si plusieurs classes sont applicables et même si toutes assignent le
même rôle à un remplisseur donné, il est impossible d'interpréter ce rôle. Au
contraire, si une seule classe est applicable, alors on peut interpréter le
rôle assigné au remplisseur. 
...
VerbNet de la phrase devient NP V NP NP.
...
\texttt{characterize-29.2} et \texttt{classify-29.10}. Les frames possibles
sont~:
...
fur et à mesure des annotations. Cette étape correspond aux \emph{probability
...
$$ \text{rôle} = \arg\max_{\text{rôle}} p(\text{rôle} \vert \text{prédicat}, \text{fonction})k$$
...
de granularité du à la différence construction des deux ressources
\citep{palmer2009semlink}. Cela implique qu'il est difficile d'utiliser un tel
mapping pour convertir des annotations en rôles sémantiques, et c'est une des
...
% TODO FUTURE SEMAFOR MXPOST/MST
...
\begin{table*}[ht]
    ...
    \caption{\protect\centering\label{table:results}Résultats pour différentes tâches. \emph{probabiliste} indique la correspondance probabiliste. \emph{passif} indique la détection de la voix passive. \emph{Identification} est l'identification des arguments.}
