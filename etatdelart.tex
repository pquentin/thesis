...
étudiant les dictionnaires, les ressources lexicales (en s'attardant sur
WordNet) et les modèles de langue.
...
lexicographe examine ses différents usages dans le corpus pour les séparer.
Cette séparation permet d'établir des critères objectifs : ces critères, une
fois établis, permettent de valider la séparation. Ils serviront aussi de base
à la définition, alors que les utilisations de mots dans le corpus pourront
servir d'exemple dans le dictionnaire. Une phase d'ajustement peut suivre pour
harmoniser les groupes obtenus étant donné des critères établis, et ce n'est
qu'après toutes ces étapes que l'écriture de l'entrée du dictionnaire peut
avoir lieu.  L'avantage principal est que le choix est désormais basé sur des
données réelles, et n'est pas de la simple spéculation. Certains sens, jugés
trop peu fréquents, sont laissés de côté, tandis que d'autres, jugés trop
proches, sont regroupés.
...
illusoire de vouloir définir un dictionnaire général parfait pour tous les sens
possibles d'un mot. Néanmoins, il n'est pas concevable de réaliser manuellement
un dictionnaire par corpus ; et il faut simplement être conscient des
difficultés théoriques posées par les sens de mots.

Des choix différents seraient faits pour un dictionnaire spécialisé qui
s'appuierait sur un corpus différent.
% donner un exemple

\subsubsection{Inventaires de sens}

La qualité du travail lexicographique des dictionnaires n'a pas été remise en
cause par l'ère numérique. Cependant, les ordinateurs rendent possibles
d'autres formats pour les dictionnaires, la première raison étant qu'il n'est
plus nécessaire de les trier par ordre alphabétique pour pouvoir identifier un
mot rapidement \citep{miller1985wordnet}, la seconde raison étant que le
lexique est mieux représenté par un graphe que par un texte
\citep{polguere2012lexicographique}.

% pas de biais lexicographique ?
