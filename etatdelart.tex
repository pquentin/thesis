...
La question du sens des mots occupe une place importante dans nos travaux,
notamment pour la traduction des ressources lexicales (Chapitres~\ref{ch:wonef}
et \ref{ch:verbnet}). Nous commençons par étudier des critères permettant de
distinguer deux sens d'un même mot afin d'arriver à la création d'un inventaire
de sens dit «~discret~» dans lequel les différents sens d'un mot sont tous
distincts. Dans cette première partie, nous nous éloignons de toute
considération sur les approches cognitives de la représentation du sens et de
la modélisation du cerveau \citep{harnad1990symbol} pour nous concentrer, donc,
sur des inventaires de sens finis.

Ensuite, nous présentons deux familles de modèles de langue : les modèles à
base de n-grams et les modèles neuronaux. Enfin, nous étudions la question de
la similarité entre deux mots et deux groupes de mots à l'aide de ces modèles
de langue.


\subsection{Distinction de deux sens}

Nous nous posons ici la question de la distinction entre deux sens. Si deux
sens considérés ne peuvent pas être distingués, alors ils appartiennent à la
même entrée dans notre inventaire de sens. La réponse à cette question est une
première étape vers la création d'un inventaire de tous les sens d'un mot.

\subsubsection{Homonymie et polysémie}

La première question est de savoir si deux sens sont en relation d'homonymie ou
de polysémie. Considérons ces deux sens de la définition du mot « sol » dans le
\emph{Dictionnaire de l'Académie Française, huitième édition} :
...
Pour la phrase \emph{Le problème des sols pollués ne fait pas recette auprès du
grand public}, le sens correct n'est pas clair et deux annotateurs risquent
d'être en désaccord étant donné que les deux sens de la définition rentrent en
jeu dans cette phrase. Il est possible de construire une telle phrase ambigüe
parce que le mot est polysémique. En effet, les deux sens cités ci-dessus
partagent la même étymologie (du latin \textit{solum}).

Au contraire, un autre sens de sol («~Cinquième note de la gamme
d'\textit{ut}~») a lui sa propre étymologie (syllabe d'un poème du
VIII\textsuperscript{ème} siècle). Les sens « note de musique » et « surface »
sont eux liés par une relation d'homonymie et sont très faciles à distinguer.
\cite{yarowsky1995unsupervised} obtient d'ailleurs d'excellents résultats sur
la tâche de désambiguïsation lexicale en se limitant à la distinction de
quelques homonymes. Certains sens, bien que partageant la même étymologie et
n'étant donc pas homonymes, sont aussi très faciles à distinguer . L'entrée
dans un lieu et l'entrée d'un dictionnaire partagent la même origine, mais sont
...
des mots présents dans ce dictionnaires.

La suite de cette section ne considère pas explicitement ces relations
homonymes ou proches de l'homonymie pour se concentrer sur les distinctions
plus subtiles et en entraînant le plus de désaccords.

\subsubsection{Traductions}

Un premier moyen de distinguer deux sens est de considérer une application qui
se servira de cette distinction, afin de s'adapter au mieux à cette
application. La traduction automatique est un bon exemple, et donne ce critère
: si un mot dans une langue source a plusieurs traductions dans une langue
cible, alors il faut distinguer ces deux sens \citep{resnik1997perspective}.
Par exemple, « entrée » est traduit par « entry » ou « entrance » suivant qu'il
réfère à l'entrée d'un dictionnaire ou d'un lieu. Cette approche n'est pas
parfaite : par exemple, le mot aile est traduit de la même manière en anglais,
allemand, espagnol et italien, qu'il réfère à l'aile d'un bâtiment ou l'aile
d'un oiseau.  Cependant, l'approche a l'avantage d'être objective ; et c'est la
première méthode qui est adaptée à une tâche particulière.
% TODO citations
Différents travaux ont poursuivi cette voie, en se basant notamment sur
EuroWordNet \citep{tufis2006from} et sur des corpus parallèles
\citep{chan2007nus,zhong2009word,plas2011automatic}.

\subsubsection{Test d'ambiguïté}

\cite{kilgarriff1997don} mentionne l'existence différents tests d'ambiguïté, le
plus efficace étant celui des lectures croisées. Pour savoir si les sens « main
gauche » et « main droite » doivent être différenciés dans un inventaire, on
pourra écrire la phrase suivante : « Paul a levé la main, tout comme Jean ».
Paul peut avoir levé la main gauche et Jean la main droite, ce qui signifie que
ces sens ne sont pas distincts. Pour distinguer le sens « jeu » et « papillon »
du mot « échiquier », on peut penser à la phrase « Paul est arrivé avec un
échiquier, tout comme Jean ». Ici, il n'est pas \textit{a priori} possible que
Paul soit arrivé avec un jeu, et Jean avec un papillon ; ce qui indique que ces
sens sont différents, ce qui valide l'utilisation du test.

\subsection{Concordancier sur un corpus}
\label{besoincorpus}

Cependant, les tests d'ambiguïté ne sont pas faciles à construire et ne donnent
...
\citep{mouton2010jaws}.%,nguyen2012using}.
% pas le seul inconvénient des dictionnaires
...
\subsection{Représentations continues}
\label{sensfunky}
